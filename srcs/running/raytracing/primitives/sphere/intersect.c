...
/*   Updated: 2019/01/13 19:26:03 by cvermand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (sphere->params.shape.params.sphere.radius);
}
...
float			sphere_intersect(t_ray ray, t_obj *sphere)
{
	...
	length = vec_dotproduct(ray.dir, origin_to_sphere);
	if (length <= 0)
		return (INFINITY);
	else
	{
		proj_to_center =
			powf(vec_magnitude(origin_to_sphere), 2) - powf(length, 2);
		proj_to_center = (proj_to_center < 0) ?
			sqrtf(-proj_to_center) : sqrtf(proj_to_center);
		if (is_equal_float(proj_to_center, sphere_radius(sphere)))
			return (length);
		if (proj_to_center > sphere_radius(sphere))
			return (INFINITY);
		return (length - (sqrtf(powf(sphere_radius(sphere), 2)
						- powf(proj_to_center, 2))));
	}
	return (INFINITY);
}
